...
import { addCountry } from "../redux/countryActions";
import { loading } from "../redux/countryActions";
import { cases } from "../redux/countryActions";

const Thapa = () => {
  const countryData = useSelector((state) => state.allCountries.countryData);
  const loadingEffect = useSelector(
    (state) => state.loadingEffect.loadingEffect
  );
  const coronaCases = useSelector((state) => state.coronaCases.coronaCases);
  ...
  const filterByRegion = async (continent) => {
    ...
    const res = await fetch(
      `https://disease.sh/v3/covid-19/continents/${continent}`
    );
    ...
    <>
      <Link to="/">
        ...
        <div className={dark ? "app-body white-mode" : "app-body"}>
          ...
          <div>
            {<h2 className="global-cases">Total Cases: {coronaCases.cases}</h2>}
          </div>

          <div className="container">
            <section className="gird">
              {loadingEffect ? (
                countryData.map((currElem, id) => {
                  return (
